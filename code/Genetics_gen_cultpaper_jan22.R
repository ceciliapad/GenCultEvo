...
setwd("/Users/Cecilia/Documents/PhD/African_Pygmies/Genetic_studies_pygmies/Final_Data/New genetics march 2021/SNP chip data/Patin_Jarvis/ibdne_trial/results_summary/inputs_paper")

#read input files, whole genome
mdist<-read.table("Patin_Jarvis_pairwise_pi.mdist")
mdist_id<-read.table("Patin_Jarvis_pairwise_pi.mdist.id")

dist<-read.table("Patin_Jarvis_pairwise_pi.dist")
dist_id<-read.table("Patin_Jarvis_pairwise_pi.dist.id")

#create final matrix to store pairwise pi values
# first you need a vector with the names of the diploid samples
library(stringr)
library(gdata)

#samples_names_final<-unique(mdist_id[,1])
mdist <- as.matrix(mdist)
rownames(mdist) <- mdist_id[,2]
colnames(mdist) <- mdist_id[,2]

mdist[upper.tri(mdist)] <- 42

library(reshape2)
pi_df <- melt(mdist)
pi_df <- filter(pi_df, value != 42) %>% filter(Var1 != Var2)

pi_df_groups <- pi_df
pi_df_groups$Var1 <- as.character(pi_df_groups$Var1)
pi_df_groups$Var2 <- as.character(pi_df_groups$Var2)

for (i in 1:nrow(pi_df)) {
  # first individual
  ind1 <- pi_df[i,1]
  row_group <- which(mdist_id$V2==ind1, arr.ind=TRUE)
  ind_group_1 <- mdist_id[row_group,1]
  pi_df_groups[i,1] <- ind_group_1
  # second individual
  ind2 <- pi_df[i,2]
  row_group <- which(mdist_id$V2==ind2, arr.ind=TRUE)
  ind_group_2 <- mdist_id[row_group,1]
  pi_df_groups[i,2] <- ind_group_2
}

group <- pi_df_groups %>%
  group_by(Var1, Var2) %>%
  summarise(a_mean=(mean(value)))

###################################################
## PATIN-ONLY DATASET: SEE IF POPULATIONS MATCH! ##
###################################################
## This will produce genetic descriptives we need for Gene-Culture coevolution paper
...
# read .fam file from prunned dataset (needed for PCA & FST)
fam_prunned <- read.table("GabonDiv_cleaned_KING_pruned.fam")
...
# write fam file again
write.table(fam_prunned, "GabonDiv_cleaned_KING_pruned2.fam", sep=" ", row.names = F,  col.names = F,quote=F)
...
# read .fam file from non prunned dataset (needed for HOMOZYG)
fam_non <- read.table("GabonDiv_cleaned_KING.fam")
...
# write fam file again
write.table(fam_non, "GabonDiv_cleaned_KING_grouped.fam", sep=" ", row.names = F,  col.names = F,quote=F)

# write list of cahg to keep in filtering process
cahg <- c("Biaka", "Baka", "Bakola", "Bakoya", "Babongo", "Bedzan", "Mbuti",
          "Batwa")

setwd("/Users/Cecilia/Documents/PhD/African_Pygmies/Genetic_studies_pygmies/Final_Data/New genetics march 2021/SNP chip data/Patin_Jarvis/Patin_only_results")
het=as.matrix(read.table("homozygosity_Patin.het",head=T))
het_df <- as.data.frame(het)
...
# Now FST
fst <- read.csv("output_CAHG/pairwise_fst_PO.csv", row.names = 1)
fst <- as.matrix(fst)
pygmy_names <- row.names(fst)
# edit name directly in .csv file
#pygmy_names[pygmy_names=="Biaka"] <- "Aka"
#fst[fst=="Biaka"] <- "Aka"
pygmy_names[7] <- "Efe & Sua"
colnames(fst) <- pygmy_names
rownames(fst) <- pygmy_names
fst <- fst[-c(8:9),-c(8:9)]
dim <- ncol(fst)

heatmap(fst, scale = "none", col=hcl.colors(10, "Geyser", rev=TRUE))
text(expand.grid(1:dim, 1:dim), sprintf("%0.2f", fst), cex=0.6)
notes_fst <- round(fst,3)

library("gplots")
png(file="inputs_paper/fst_3dp.png",
    width=7, height=7, units="in", res=500)
heatmap.2(fst, # to build the heat map you need all digits as otherwise you'll get blank cells
          cellnote = notes_fst,  # same data set for cell labels
          main = "Fst", # heat map title
...
pca1 <- read.table("pca_GabonDiv_CAHG_groups.eigenval",sep=" ",header=F)
pca2 <- read.table("pca_GabonDiv_CAHG_groups.eigenvec",sep=" ",header=F)

#pca1 <- read.table("/Users/Cecilia/Documents/PhD/African_Pygmies/Genetic_studies_pygmies/Final_Data/New genetics march 2021/SNP chip data/Patin_Jarvis/pca_groups.eigenval",sep=" ",header=F)
#pca2 <- read.table("/Users/Cecilia/Documents/PhD/African_Pygmies/Genetic_studies_pygmies/Final_Data/New genetics march 2021/SNP chip data/Patin_Jarvis/pca_groups.eigenvec",sep=" ",header=F)

pca2[pca2=="Biaka"]<-"Aka"
pca2[pca2=="Mbuti"]<-"Efe & Sua"
population=as.data.frame(table(pca2$V1))$Var1
sample.id = pca2$V1
...
my_pal <- my_pal_long[-c(4,7,10,11)]
...
roh <- read.table("homozygosity_CAHG_Patin.hom",sep="", header = T)
roh_ind <- read.table("homozygosity_CAHG_Patin.hom.indiv",sep="", header = T)
...
roh_pops_2 <- roh_pops_2 %>%                 # Specify data frame
  ...
  summarise_at(vars(KB),                     # Total Genome in Runs of homozygosity
               list(mean_hom = mean,
                    sd_hom = sd,
                    median_hom= median))            # Specify function
write.csv(roh_pops_2, "inputs_paper/runs_homo.csv")
pygmies_roh$KB  <- as.integer(round(pygmies_roh$KB,0))
...
p <- ggplot(pygmies_roh, aes(x=FID, y=KB, fill=FID)) + 
  ...
  geom_boxplot(width=0.1, outlier.shape = NA, fill="white")+
  theme_classic() 
p <- p + scale_fill_manual(values=my_pal) + theme(legend.position = "none", text = element_text(size = 12))
ggsave(p, filename="inputs_paper/runs_homo_pygmies_new.png", height=150, width=250, units="mm")
